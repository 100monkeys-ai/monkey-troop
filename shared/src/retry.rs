    ...
    Fut: std::future::Future<Output = TroopResult<T>>,
{
    ...
    for attempt in 0..MAX_RETRIES {
        match operation().await {
            Ok(result) => {
                if attempt > 0 {
                    eprintln!("{} succeeded on retry attempt {}", operation_name, attempt + 1);
                }
                return Ok(result);
            }
            Err(e) => {
    ...
    Err(last_error.unwrap_or(TroopError::InternalError(
        "Unknown retry error".to_string()
    )))
}
...
mod tests {
    ...
    async fn test_retry_succeeds_immediately() {
        let result = retry_with_backoff("test_op", || async {
            Ok::<_, TroopError>(42)
        }).await;
