...
use monkey_troop_shared::{
    CircuitBreaker, HardwareInfo, NodeHeartbeat, NodeStatus, CIRCUIT_BREAKER_THRESHOLD,
...
pub async fn run_heartbeat_loop(
    ...
    info!(
        "Starting heartbeat loop (every {}s)",
    ...
    loop {
        ...
        let should_refresh =
            last_model_refresh.elapsed().as_secs() >= config.model_refresh_interval;
