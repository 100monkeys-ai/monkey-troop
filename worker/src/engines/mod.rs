...
pub fn build_model_registry(engines: &[Box<dyn EngineDriver + Send + Sync>]) -> Result<ModelRegistry> {
        ...
        for engine in engines {
            ...
            if info.engine_type == *priority_type {
                ...
                for model in models {
                    ...
                    registry.model_to_engine.entry(model.clone()).or_insert(base_url.clone());
                }
            }
        }
    }
    ...
    println!("📋 Registered {} models across {} engines", 
