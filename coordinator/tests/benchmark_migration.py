...
def benchmark_migration(num_users=1000, nodes_per_user=2):
    ...
    with engine.begin() as conn:
        ...
        subquery = sa.select(users_table.c.id).where(users_table.c.public_key == nodes_table.c.owner_public_key).scalar_subquery()
        conn.execute(
            sa.update(nodes_table)
            .values(owner_id=subquery)
        )
    ...
    with engine.connect() as conn:
        count = conn.execute(sa.select(sa.func.count()).select_from(nodes).where(nodes.c.owner_id == None)).scalar()
        if count > 0:
