...
class AuditService:
    ...
    def _write_to_db(self, event_type: str, user_id: Optional[str], ip_address: str, details: Dict[str, Any]):
        try:
    ...
    def log_event(self, event_type: str, user_id: Optional[str], ip_address: str, details: Dict[str, Any]):
        # Write to file
        audit_logger.info(json.dumps({**details, "event": event_type}))
        # Write to DB
        self._write_to_db(event_type, user_id, ip_address, details)

    def get_logs(self, limit: int = 100, offset: int = 0, event_type: Optional[str] = None, user_id: Optional[str] = None) -> List[Dict[str, Any]]:
